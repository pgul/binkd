...
Со всеми предложениями и замечаниями пишите: Stas Degteff 2:5080/102
    ...
    23. Что означает диагностика "send: TCP/IP error (-10000)", как это лечить?
    24. Argus (Radius) и binkd: ошибка Argus "Aborting due to carrier loss"
    25. В названии аутбаунда - символ комментария и binkd не видит каталог.
    26. Есть ли возможность запуска приложения в binkd по событию?
    27. В чем различие между binkd/w32 и binkd/w9x?
    28. Сильно не хватает чата в binkd! И время синхронизировать хочется.
    29. Binkd принимает входящее соединение только после исходящего.
    30. Ошибка "start_file_transfer: ECHO is off.: No such file or directory".
    31. Как правильно создать полл.
    32. Что означают цифpы в квадpатных скобках в логе?
    33. Как настроить эхотаг для отправки исходящего нетмейла?
    34. Зачем нужна директива share в конфиге (shared aka)?
    35. Зачем нужна директива ftrans в конфиге?
    36. Почему binkd в Windows XP работает сервисом только от администратора?
    37. Кто-нибудь прикрутит, наконец поддержку нодлиста к бинкд?
...
----------------------------------------------------------------------------
    ...
    http://krts.pp.ru/ftp/fido/statddos.rar
...
----------------------------------------------------------------------------
    ...
    21. Разные FTN-домены в binkd и тоссер, не поддерживающий 5D outbound.
...
----------------------------------------------------------------------------
    ...
    23. Что означает диагностика "send: TCP/IP error (-10000)" и как это лечить?
...
----------------------------------------------------------------------------
    ...
    25. В названии каталога - символ комментария и binkd не видит каталог.
...
----------------------------------------------------------------------------
    ...
    29. Binkd-service принимает входящее соединение только после исходящего.
...
(для пользователя username и сервиса с именем binkd-service):
    ...
    Подробнее см. http://support.microsoft.com/kb/288129/en-us (Windows 2000)
и http://support.microsoft.com/kb/325349/en-us (Windows 2003)
...
будет применен, и binkd будет поддерживать нодлист native.
