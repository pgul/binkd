...
фрек-процессора, поддерживающего SRIF (к примеру, Allfix). Читай файл
"!SRIF.TXT" и комментарий к "exec" в конфиг-файле.
...
----------------------------------------------------------------------------
    ...
    22. Разные FTN-домены в binkd и тоссер, не поддежривающий 5D outbound.
...
возникнуть, если отсутствует поддержка 5D outbound у тоссера, трекера и других
...
завершении (перезагрузке) системы. Кроме того, в Windows 9x невозможно спрятать
консоль.
...
планируется сделать поддержку сервисов NT в binkd/w9x. Поддержка работы 
binkd/w32 сервисом под Windows 9x невозможна из-за описанных особенностей
реализации консоли в этих системах.
